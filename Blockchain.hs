...
guessNonce :: Hash -> Transaction -> Hash -> Hash
guessNonce p c r = (n - (h `mod` n)) `mod` n
  where
    n = 2^difficulty
    h = hash [p,hash c, r, 0]

-- In guessNonce we used how hash is calculated, if hash is changed we can
-- brute force nonce since there are only 2^difficulty possibilities.
forceNonce :: Hash -> Transaction -> Hash -> Hash -> Hash
forceNonce p c r n = if validNonce (BlockHeader p c r n) then n
                      else forceNonce p c r (n+1)

mineBlock :: Miner -> Hash -> [Transaction] -> Block
mineBlock miner parent txs = Block header txs
    where
      header = BlockHeader parent coinbase root nonce
      root = treeHash $ buildTree (coinbase:txs)
      coinbase = coinbaseTx miner
      nonce = guessNonce parent coinbase root
...
validChain = isNothing . verifyChain

verifyChain :: [Block] -> Maybe Hash
verifyChain xs = helper 0 $ reverse xs
  where helper h (x:xs) = if isNothing $ verifyBlock x h then Nothing
                          else helper (hash x) xs
        helper h [] = Just h
...
mineTransactions miner parent txs = (block, rpts)
  where
  block = mineBlock miner parent txs
  cb = coinbase (blockHdr block)
  tree = buildTree $ cb:txs
  proof x = buildProof x tree
  rpts = reverse $ buildReceipts txs tree (hash block)

buildReceipts :: [Transaction] -> Tree Transaction -> Hash -> [TransactionReceipt]
buildReceipts [] _ _ = []
buildReceipts (x:xs) tree h = let proof = buildProof x tree in
  case proof of
    Just p -> TxReceipt h p : buildReceipts xs tree h
    Nothing -> buildReceipts xs tree h
